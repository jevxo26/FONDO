...
export const createCoupon = catchServiceAsync(async (data: Record<string, unknown>) => {
  const existing = await prisma.coupon.findUnique({
    where: { couponCode: data.couponCode as string },
  });
  ...
  return prisma.coupon.update({
    ...
    data: {
      ...
      ...(data.minimumOrderAmount !== undefined && {
        minimumOrderAmount: data.minimumOrderAmount as number | null,
      }),
      ...(data.maximumDiscount !== undefined && {
        maximumDiscount: data.maximumDiscount as number | null,
      }),
