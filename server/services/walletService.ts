...
export const listTransactions = catchServiceAsync(async (customerId: string) => {
  ...
  return prisma.customerWalletTransaction.findMany({
    where: { walletId: wallet.id },
    orderBy: { createdAt: "desc" },
  });
});

export const topupWallet = catchServiceAsync(
  async (customerId: string, data: { amount: number; paymentMethodId?: string }) => {
    const wallet = await getOrCreateWallet(customerId);
      ...
      if (validation.validated) {
        await prisma.$transaction([
          prisma.walletTopup.update({
            where: { id: topupId },
            data: { status: "completed", paymentId: tran_id },
          }),
          prisma.customerWallet.update({
            where: { id: topup.wallet.id },
            data: { balance: { increment: Number(topup.amount) } },
          }),
...
export const requestWithdraw = catchServiceAsync(
  async (
    customerId: string,
    data: { amount: number; withdrawMethod: string; accountNumber: string },
  ) => {
    const wallet = await getOrCreateWallet(customerId);
...
export const approveWithdraw = catchServiceAsync(async (adminId: string, withdrawId: string) => {
  ...
  await prisma.$transaction([
    prisma.walletWithdraw.update({
      where: { id: withdrawId },
      data: { status: "approved", approvedBy: adminId, processedAt: new Date() },
    }),
    prisma.customerWallet.update({
      where: { id: withdraw.wallet.id },
      data: {
        holdBalance: { decrement: Number(withdraw.amount) },
        balance: { decrement: Number(withdraw.amount) },
      },
    }),
    prisma.customerWalletTransaction.create({
      data: {
