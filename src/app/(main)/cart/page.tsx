...
export default function CartPageView() {
  ...
  const subtotalValue = cartItems.reduce((acc, item) => acc + (item.oldPrice || item.price) * item.quantity, 0);
        ...
        <div className="wrapper">
          ...
          {cartItems.length > 0 ? (
            ...
            <div className="py-16 text-center border border-dashed border-border rounded-[32px] bg-white dark:bg-card">
              ...
              <Link href="/menu" className="mt-4 inline-flex h-10 items-center rounded-xl bg-[#CEA359] px-5 font-sans text-xs font-semibold text-[#1B0E08]">
                Return to Menu
              </Link>
            </div>
          )}
