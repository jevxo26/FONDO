import { NutritionTableSection } from "@/components/dashboard/admin/foods/nutrition/nutrition-table-section";
import { PageHeader } from "@/components/dashboard/common/page-header";
import { StatCard } from "@/components/dashboard/common/stat-card";
import { nutritionItems } from "@/data/nutrition";
import { DollarSign, Scale, TrendingDown, Zap } from "lucide-react";

export default function FoodsNutritionPage() {
  const avgCalories = Math.round(
    nutritionItems.reduce((s, i) => s + i.calories, 0) / nutritionItems.length,
  );
  const avgProtein = Math.round(
    nutritionItems.reduce((s, i) => s + i.protein, 0) / nutritionItems.length,
  );
