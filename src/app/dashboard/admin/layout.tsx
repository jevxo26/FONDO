...
import { sidebarItems, sectionConfig } from "@/data/dashboard";
import { DashboardBgPattern } from "@/components/dashboard/layout/bg-pattern";
...
export default function DashboardLayout({
  ...
  return (
    <SidebarProvider className="bg-[radial-gradient(ellipse_at_top_right,color-mix(in_srgb,var(--primary)_6%,transparent)_0%,transparent_65%)]">
      <DashboardSidebar
        items={sidebarItems}
      ...
      >
        <DashboardBgPattern variant="admin" />
        <DashboardHeader />
        <div className="relative z-10 grid w-full grid-cols-1 xl:grid-cols-[minmax(0,1fr)_minmax(0,1920px)_minmax(0,1fr)] xl:grid-rows-1">
          <div className="pointer-events-none relative hidden h-full bg-gradient-to-r from-primary/[0.04] to-transparent xl:block">
            <div className="absolute inset-0 opacity-[0.08] bg-[repeating-linear-gradient(45deg,transparent,transparent_32px,color-mix(in_srgb,var(--primary)_12%,transparent)_32px,color-mix(in_srgb,var(--primary)_12%,transparent)_33px)]" />
          </div>
          ...
          <div className="pointer-events-none relative hidden h-full bg-gradient-to-l from-primary/[0.04] to-transparent xl:block">
            <div className="absolute inset-0 opacity-[0.08] bg-[repeating-linear-gradient(135deg,transparent,transparent_32px,color-mix(in_srgb,var(--primary)_12%,transparent)_32px,color-mix(in_srgb,var(--primary)_12%,transparent)_33px)]" />
          </div>
        </div>
      </SidebarInset>
    </SidebarProvider>
