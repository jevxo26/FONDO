...
export default function ReportsInventoryPage() {
  ...
  const topCategory = [
    ...inventoryReportData.reduce((acc, i) => {
    ...
    <div>
      ...
      <div className="mt-12 grid grid-cols-1 gap-6 lg:grid-cols-3">
        <DarkCard
          ...
          title="Out of Stock"
          description={`${outOfStock} items need immediate action`}
          className="flex-1"
        >
          <div className="font-heading text-4xl font-bold drop-shadow-[0_2px_4px_rgba(0,0,0,0.4)] text-red-400">
            {outOfStock}
          </div>
        ...
        <GlassCard
          ...
          iconBg="bg-muted"
          title="Overstocked"
          value={overstocked.toString()}
          subtitle="Items exceeding 3x min stock"
          className="flex-1"
        >
          <div className="mt-4 rounded-lg bg-muted px-3 py-2 text-xs text-muted-foreground">
            Top category: {topCategory[0]} ({topCategory[1]} items) · Avg daily usage: {avgUsage}{" "}
            units
          </div>
        </GlassCard>
      </div>
