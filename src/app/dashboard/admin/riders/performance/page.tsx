...
export default function RidersPerformancePage() {
  ...
  const avgOnTime = Math.round(
    riderPerformanceData.reduce((s, r) => s + r.onTimeRate, 0) / riderPerformanceData.length,
  );
  const avgRating = (
    riderPerformanceData.reduce((s, r) => s + r.avgRating, 0) / riderPerformanceData.length
  ).toFixed(1);
  const totalComplaints = riderPerformanceData.reduce((s, r) => s + r.complaints, 0);
  const topRider = riderPerformanceData.reduce((best, r) =>
    r.deliveriesThisWeek > best.deliveriesThisWeek ? r : best,
  );
      ...
      <div className="mt-12 grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        >
          <div className="mt-4 text-xs text-muted-foreground">
            Avg {(totalDeliveries / riderPerformanceData.length).toFixed(0)} deliveries per rider
          </div>
        </GlassCard>
        <GlassCard
          ...
          title="Best On-Time"
          value={`${topRider.onTimeRate}%`}
          subtitle={topRider.name}
        >
          <div className="mt-4 rounded-lg bg-muted px-3 py-2 text-xs text-muted-foreground">
            {topRider.deliveriesThisWeek} deliveries this week with {topRider.lateDeliveries} late
          </div>
        </GlassCard>
        <GlassCard
          icon={<Award className="size-5 text-warning" />}
          iconBg="bg-warning/10"
          title="Complaint Rate"
          value={`${((totalComplaints / totalDeliveries) * 100).toFixed(1)}%`}
          subtitle="Of total deliveries"
        >
          <div className="mt-4 rounded-lg bg-muted px-3 py-2 text-xs text-muted-foreground">
            {totalComplaints} complaints out of {totalDeliveries} deliveries
          </div>
        </GlassCard>
      </div>
    </div>
