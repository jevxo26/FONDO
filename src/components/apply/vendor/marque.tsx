      ...
      <div className="flex overflow-hidden space-x-12 select-none py-2">
        <div className="flex space-x-12 animate-marquee shrink-0 items-center">
          {MARQUEE_LOGOS.concat(MARQUEE_LOGOS).map((logo, index) => (
            <div key={index} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-background border border-border text-xs font-bold text-foreground opacity-80 hover:opacity-100 transition-opacity whitespace-nowrap shadow-[var(--shadow-card)]">
