...
>(({ label, icon, rightAction, ...props }, ref) => (
  <div className="flex flex-col gap-1.5 w-full font-sans text-xs">
    <label className="text-[10px] font-bold uppercase tracking-widest text-primary-foreground/70">
      {label}
    </label>
    <div className="relative">
      {icon && (
        <div className="absolute left-3.5 top-1/2 -translate-y-1/2 text-primary-foreground/40">
          {icon}
        </div>
      )}
      <input
