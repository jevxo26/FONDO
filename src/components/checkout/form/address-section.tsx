...
export function AddressSection({ addresses, selectedAddressId, onSelect, register, errors, showNewAddress, fulfillment }: Props) {
  return (
      ...
      {fulfillment === "delivery" && addresses.length > 0 && (
        <div className="flex flex-col gap-3">
          ...
          {addresses.map((addr) => (
            ...
            >
              <MapPin className={`size-4 shrink-0 ${selectedAddressId === addr.id ? "text-primary" : "text-muted-foreground"}`} />
              <div className="flex-1 min-w-0">
                ...
                <p className="font-sans text-[11px] text-muted-foreground mt-0.5 truncate">
                  {addr.road || addr.house ? `${addr.house ? addr.house + ", " : ""}${addr.road ? addr.road + ", " : ""}` : ""}
