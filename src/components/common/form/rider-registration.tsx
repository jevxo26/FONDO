...
export function RiderRegistrationForm() {
  const { register, handleSubmit, watch, formState: { errors, isSubmitting } } = useForm<RiderFormData>({
    resolver: yupResolver(riderFormSchema),
    defaultValues: {
      fullName: "", phone: "", email: "", nidNumber: "", dob: "",
      emergencyName: "", emergencyPhone: "", emergencyRelation: "",
      workZone: "", vehicleType: "motorbike", drivingLicenseNo: "", vehicleRegNumber: "",
      payoutMethod: "bkash", mobileWalletNumber: "", bankName: "", bankAccountNumber: "",
      termsAccepted: false, safetyCodeAccepted: false, backgroundCheckAccepted: false,
    },
  });
  ...
  return (
    <section id="rider-apply" className="py-[var(--space-section)] bg-background border-b border-border">
      <div className="wrapper px-[var(--space-container)]">
        <div className="max-w-4xl mx-auto space-y-8">
          <div className="text-center space-y-2">
            <span className="text-xs font-bold uppercase tracking-wider text-primary">Join the Fleet</span>
            <h2 className="font-heading text-3xl sm:text-5xl tracking-tight text-foreground">Rider Registration Form</h2>
            <p className="text-xs sm:text-sm text-muted-foreground max-w-lg mx-auto">Complete your details to start delivering healthy meals in your preferred zone.</p>
          </div>
          <form onSubmit={handleSubmit(onSubmit)} className="bg-card border border-border rounded-4xl p-6 sm:p-10 shadow-[var(--shadow-elevated)] space-y-12">
            ...
            <ZoneVehicleInfo register={register} errors={errors} selectedVehicle={selectedVehicle} />
