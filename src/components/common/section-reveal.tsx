  ...
  const item: Variants = (() => {
    switch (variant) {
      case "fadeScale":
        return {
          hidden: { opacity: 0, scale: 0.9 },
          visible: { opacity: 1, scale: 1, transition: itemTransition },
        };
      case "clipReveal":
        return {
          hidden: { opacity: 0, clipPath: "inset(0 100% 0 0)" },
          visible: { opacity: 1, clipPath: "inset(0 0% 0 0)", transition: itemTransition },
        };
      case "rotateReveal":
        return {
          hidden: { opacity: 0, x: slideX, y: slideY, rotateX: 15 },
          visible: { opacity: 1, x: 0, y: 0, rotateX: 0, transition: itemTransition },
        };
      default:
        return {
          hidden: { opacity: 0, x: slideX, y: slideY },
          visible: { opacity: 1, x: 0, y: 0, transition: itemTransition },
        };
    }
  })();
