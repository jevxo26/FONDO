...
import { useCreatePackage, useGetPackageCategoriesQuery, useUpdatePackage } from "@/store/api/slices/packages-api";
...
interface Props {
  initialData?: Partial<PackageFormValues & { id?: string }>;
  isEdit?: boolean;
}

export default function PackageRegistrationForm({ initialData, isEdit }: Props) {
  ...
  return (
    <section className="py-6 lg:py-8 bg-background">
      <div className="wrapper max-w-6xl mx-auto space-y-8 px-4">
        <HeaderBar
          onReset={() => reset(initialData ? { ...initialValues, ...initialData } : initialValues)}
