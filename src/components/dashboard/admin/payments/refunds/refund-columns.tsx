...
export const refundColumns: ColumnDef<Payment>[] = [
  {
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <span className="font-mono text-xs text-muted-foreground">{row.original.order?.orderNumber ?? row.original.orderId.slice(0, 8)}</span>
    ),
  },
  {
    accessorKey: "amount",
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Badge variant="outline" className={`ring-1 ${badge.className}`}>{badge.label}</Badge>
      );
    },
  },
  {
