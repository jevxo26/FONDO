    ...
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
      ...
      >
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-white/70">of {total} total applications</p>
        </div>
      ...
      <GlassCard
        ...
        title="Rejected Rate"
        value={
          pendingVendors.filter((v) => v.status === "Rejected").length === 0
            ? "0%"
            : `${Math.round((pendingVendors.filter((v) => v.status === "Rejected").length / total) * 100)}%`
        }
