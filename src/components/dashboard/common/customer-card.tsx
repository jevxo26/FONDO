...
> = {
  bronze: {
    ...
    cardBg:
      "bg-gradient-to-br from-amber-100/80 via-card to-amber-50/50 dark:from-amber-900/40 dark:via-card dark:to-amber-950/30",
  },
  silver: {
    label: "Silver",
    badge: "bg-muted text-muted-foreground",
    cardBg:
      "bg-gradient-to-br from-slate-100/70 via-card to-slate-50/40 dark:from-zinc-800/50 dark:via-card dark:to-zinc-900/40",
  },
  gold: {
    label: "Gold",
    badge: "bg-primary/10 text-primary",
    cardBg:
      "bg-gradient-to-br from-primary/15 via-card to-primary/[0.06] dark:from-amber-800/30 dark:via-card dark:to-amber-900/20",
  },
};
...
export function CustomerCard({ customer }: CustomerCardProps) {
  ...
  const tier: Tier =
    customer.totalOrders >= 100 ? "gold" : customer.totalOrders >= 50 ? "silver" : "bronze";
  ...
  const progressLabel =
    tier === "gold"
      ? "Gold tier"
      : tier === "silver"
        ? `${100 - customer.totalOrders} to Gold`
      ...
      <div className="relative z-10 p-5 md:p-6">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <h4 className="truncate font-semibold text-foreground transition-colors duration-300 group-hover:text-primary">
                {customer.fullName}
              </h4>
          ...
          <span
            className={`shrink-0 rounded-full px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${config.badge}`}
          >
            {config.label}
          </span>
        </div>
        ...
        <div className="mt-5 grid grid-cols-3 gap-3">
          ...
          <div className="rounded-xl bg-muted/40 p-3 ring-1 ring-primary/[0.06]">
            ...
            <span
              className={`mt-2 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-semibold ring-1 ${activePill(customer.lastActive).style}`}
            >
