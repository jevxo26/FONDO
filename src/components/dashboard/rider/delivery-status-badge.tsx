...
const statusConfig: Record<DeliveryStatus, { label: string; class: string }> = {
  ...
  ON_THE_WAY: {
    label: "On the Way",
    class: "bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300",
  },
  ARRIVED: {
    label: "Arrived",
...
export function DeliveryStatusBadge({ status }: { status: DeliveryStatus }) {
  ...
  return (
    <span
      className={cn("inline-block rounded-full px-2.5 py-0.5 text-[11px] font-semibold", cfg.class)}
    >
      {cfg.label}
    </span>
  );
