...
const FoodSingleReview = ({ review, user, setSelectedReview, setOpen }: SingleReviewProps) => {
  const isOwner = user?.id === review.customerId;
  const initials =
    `${review.customer.firstName[0] || ""}${review.customer.lastName[0] || ""}`.toUpperCase();
    ...
    >
      ...
      <div className="flex-1 space-y-2">
        ...
        <p className="text-xs text-muted-foreground font-sans font-light leading-relaxed pt-0.5">
          {review.review || (
            <span className="italic text-foreground/30">No written comment provided.</span>
          )}
        </p>
      </div>
    </div>
