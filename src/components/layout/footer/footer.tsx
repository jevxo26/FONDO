...
export function Footer() {
  return (
    <footer className="bg-foreground px-[60px] py-10 text-white">
      <div className="mx-auto max-w-[1320px] flex flex-col gap-6">
        {/* Top row */}
        <div className="flex items-start gap-[100px]">
          {/* Left: Logo + Description + Newsletter */}
          <div className="flex w-[441px] flex-col gap-6">
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-[20px] font-bold text-background">
                FONDO
              </span>
            </div>

            <p className="text-[16px] leading-[130%] text-background/65">
              A heritage kitchen in the heart of Dhaka, plating slow-cooked
              Mughlai cuisine with the precision of a modern restaurant.
            </p>

            <div className="flex items-center gap-2 rounded-full border border-background/15 bg-background/5 p-2 pr-2">
              <input
                type="email"
                placeholder="Your email for offers"
                className="flex-1 bg-transparent px-4 text-[14px] text-background placeholder:text-background/40 focus:outline-none"
              />
              <button
                type="button"
          ...
          <div className="flex gap-8">
            ...
            <div className="flex w-[237px] flex-col gap-6">
              <h4 className="text-[18px] font-bold text-background">
              ...
              <ul className="flex flex-col gap-3">
                {FOOTER_LINKS.visit.links.map((link) => (
                  <li key={link.href}>
                    <a
                      href={link.href}
                      className="text-[16px] text-background/80 transition-colors hover:text-background"
                    >
                      {link.label}
                    </a>
                  </li>
            ...
            <div className="flex w-[237px] flex-col gap-6">
              ...
              <ul className="flex flex-col gap-3">
                {FOOTER_LINKS.hours.items.map((item) => (
                  <li
                    key={item}
                    className="text-[16px] text-background/80"
                  >
                    {item}
                  </li>
                ))}
              ...
              <ul className="flex flex-col gap-4">
                {FOOTER_LINKS.contact.items.map((item) => (
                  <li key={item.text} className="flex items-center gap-3">
                    ...
                    <span className="text-[16px] text-background/80">
                      {item.text}
                    </span>
                  </li>
                ))}
              ...
              <div className="flex items-start gap-3">
                {SOCIAL_ICONS.map((icon) => (
                  <a
                    ...
                    className="flex size-10 items-center justify-center rounded-xl bg-background/10 text-background transition-colors hover:bg-background/20"
                  >
                    {icon.svg}
                  </a>
                ))}
        ...
        <div className="flex items-center justify-between border-t border-background/10 pt-8">
          <div className="flex flex-col gap-2">
            <p className="text-[14px] text-background/60">
            ...
            <div className="flex gap-3">
              {PAYMENT_METHODS.map((method) => (
                <span
                  key={method}
                  className="rounded border border-background/20 bg-background/10 px-3 py-1.5 text-[12px] font-medium text-background"
                >
                  {method}
                </span>
              ))}
          ...
          <div className="flex flex-col items-end gap-1">
            <p className="text-[14px] text-background/80">
              &copy; 2026 Finora Commerce. All rights reserved.
            </p>
            <p className="text-[12px] text-background/60">
