          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              ...
              <p className="text-[11px] text-muted-foreground/70">
                Tailor duration frameworks, allocations, and premium add-ons
              </p>
            </div>
          </div>
          <ChevronDown
              ...
              <div className="grid grid-cols-3 gap-3">
                {[7, 15, 30].map((d) => (
                  <button
                    key={d}
                    type="button"
                    className={`py-3 rounded-xl border text-xs font-bold transition-all ${d === 30 ? "border-primary bg-primary/5 text-primary" : "border-border/40 bg-transparent"}`}
                  >
                    {d} Day Ritual
                  </button>
                ))}
