      ...
      <div className="wrapper space-y-6">
        <div className="text-center">
          <h2 className="font-heading text-xl md:text-3xl font-normal text-foreground">
            Food Flow in the Wild
          </h2>
          <p className="font-sans text-xs text-muted-foreground mt-1">
            Real snapshots shared by our culinary collective tags
          </p>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {images.map((src, idx) => (
            <div
              key={idx}
              className="relative aspect-square rounded-2xl overflow-hidden border border-border bg-card shadow-[var(--shadow-card)] group"
            >
              <Image
                width={600}
                height={600}
                src={src}
                alt="Subscriber lunch plating profile"
