...
export function getOrderDetail(orderId: string): ExtendedOrderDetail {
  ...
  const detail: ExtendedOrderDetail = {
    ...
    rider:
      rand() > 0.4
        ? ["Kamal Hossain", "Rafiqul Islam", "Shahidul Haque", "Nurul Islam"][
            Math.floor(rand() * 4)
          ]
        : null,
    timeline: [
      {
        label: "Order Placed",
        time: `${baseHour}:${String(baseMin).padStart(2, "0")} PM`,
        done: true,
      },
      {
        label: "Confirmed",
        time: `${baseHour}:${String(baseMin + 5).padStart(2, "0")} PM`,
        done: true,
      },
      {
        label: "Assigned to Kitchen",
        time: `${baseHour}:${String(baseMin + 10).padStart(2, "0")} PM`,
        done: true,
      },
      {
        label: "In Preparation",
        time: `${baseHour}:${String(baseMin + 15).padStart(2, "0")} PM`,
        done: rand() > 0.3,
      },
      {
        label: "Ready for Pickup",
        time: `${baseHour + 1}:${String(baseMin).padStart(2, "0")} PM`,
        done: false,
      },
