...
export const riderFormSchema = yup.object({
  ...
  phone: yup.string().matches(/^01[3-9]\d{8}$/, "Valid 11-digit phone required").required("Phone required"),
  email: yup.string().email("Enter a valid email").required("Email required"),
  nidNumber: yup.string().matches(/^\d{10,17}$/, "NID must be 10-17 digits").required("NID is required"),
  dob: yup.string().required("Date of birth is required"),

  emergencyName: yup.string().required("Emergency contact name required"),
  emergencyPhone: yup.string().matches(/^01[3-9]\d{8}$/, "Valid 11-digit phone required").required("Emergency phone required"),
  ...
  mobileWalletNumber: yup.string().matches(/^01[3-9]\d{8}$/, "Valid 11-digit number required").required("Wallet number required"),
  ...
  backgroundCheckAccepted: yup.boolean().oneOf([true], "Must consent to background check").required(),
});
