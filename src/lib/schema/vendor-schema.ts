...
export const vendorFormSchema = yup.object({
  ...
  businessPhone: yup.string().matches(/^01[3-9]\d{8}$/, "Valid 11-digit phone required").required("Phone required"),
  ...
  ownerPhone: yup.string().matches(/^01[3-9]\d{8}$/, "Valid 11-digit phone required").required("Phone required"),
  ownerEmail: yup.string().email("Enter valid email").required("Email required"),
  nid: yup.string().matches(/^\d{10,17}$/, "NID must be 10-17 digits").required("NID required"),
  ...
  mobileBankingNumber: yup.string().matches(/^01[3-9]\d{8}$/, "Valid mobile number required").required("Number required"),
